...
export default function GuidesView({ language }: GuidesViewProps) {
  ...
  // Interactive diagnostic helper state (Simulating testing connectivity of printer)
  ...
  const GUIDES: GuideItem[] = [
    ...
    },
    {
      ...
      excerpt: isTH ? 'ขั้นตอนเชื่อมพอร์ต RJ11 ของลิ้นชักเข้าเครื่องพิมพ์ใบเสร็จ สั่งให้ลิ้นชักเด้งเด้งเปิดออโต้เมื่อพิมพ์บิลสำเร็จ' : 'Guide for connecting cash drawer RJ11 ports directly to thermal printers.',
      readTime: '5 นาที',
      content: [
    ...
    },
    {
      ...
      excerpt: isTH ? 'วิธีกำหนดสัดส่วนหน่วย ลัง, แพ็ค, ชิ้น ให้ตัดยอดสต็อกได้อย่างสมบูรณ์แบบโดยไม่ต้องสับสน' : 'Set up relationship mappings between boxes, packs, and loose items.',
      readTime: '4 นาที',
      content: [
        ...
        isTH ? 'ขั้นตอนที่ 4: เมื่อทำการขายปลีกกระป๋อง ระบบจะทำการหักย่อยเศษสต็อกอัตโนมัติรวดเร็ว' : 'Step 4: When selling single cans, parent stock automatically subtracts correctly.'
      ]
    },
    {
      id: 'g4',
      ...
      excerpt: isTH ? 'อธิบายการเปิดระบบอัจฉริยะวิเคราะห์สินค้าหมดสต็อกและทำนายช่วงเวลาที่ลูกค้าซื้อของเยอะด้วย AI' : 'Utilize integrated neural algorithms to forecast inventory replenishment.',
      readTime: '6 นาที',
      content: [
        ...
        isTH ? 'ขั้นตอนที่ 4: แสดงแผนภาพพยากรณ์สินค้าขายดี ยอดสต็อกที่ควรตุนไว้ และรายงานสินค้าใกล้หมดสต็อกล่วงหน้า' : 'Step 4: Get detailed graphs on items needing restock or seasonal peaks.'
      ]
    }
  ];
  ...
  return (
    <div className="space-y-16 pb-20">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        <div className="text-center max-w-2xl mx-auto space-y-3 mb-12">
          ...
          <h1 className="text-3xl font-sans font-extrabold text-slate-900 tracking-tight leading-tight">
            {isTH ? 'คู่มือแนะนำและการเซ็ตอัพระบบ' : 'Guides, Video Tutorials & Manuals'}
          </h1>
          <p className="text-slate-500 text-sm">
            {isTH ? 'ค้นหาบทเรียนการใช้งาน วิธีตั้งค่า และระบบตรวจสอบด้วยทีมวิศวกร' : 'Learn setup tricks, inventory organization methods.'}
          </p>
        </div>
        ...
        <div className="max-w-2xl mx-auto bg-white p-4 rounded-2xl border border-slate-200 shadow-md space-y-4">
          <div className="relative">
            ...
            <input 
              ...
              placeholder={isTH ? 'ค้นหาคำแนะนำ เช่น สต็อก, เครื่องพิมพ์, คิวอาร์...' : 'Search for setup guides...'}
          ...
          <AnimatePresence mode="popLayout">
            {filteredGuides.map((guide) => (
              ...
              >
                ...
                <div className="pt-5 border-t border-slate-50 mt-4 flex justify-between items-center">
                  ...
                  >
                    <span>{isTH ? 'เปิดอ่านบทความคู่มือ' : 'Read Guide'}</span>
            ...
            {filteredGuides.length === 0 && (
              <div className="col-span-2 py-12 text-center text-slate-400">
                ...
                <p className="text-sm font-semibold">{isTH ? 'ไม่พบหัวข้อคำแนะนำที่ตรงกับการค้นหา' : 'No matching tutorials found'}</p>
              </div>
            )}
          </AnimatePresence>
        </div>
      ...
      {/* 3. INTERACTIVE DIAGNOSTIC SIMULATOR (Cash Drawer test bench) */}
        ...
        <div className="bg-slate-900 text-white rounded-3xl p-6 sm:p-8 border border-slate-800 shadow-xl space-y-6">
          ...
          <div className="space-y-2 text-center sm:text-left">
            ...
            <p className="text-slate-400 text-xs leading-relaxed">
              {isTH 
                ? 'จำลองทดสอบเครื่องพิมพ์ความร้อนแบบเรียลไทม์เพื่อส่งสัญญาณตรวจสอบวงจร RJ11 ของลิ้นชักอัตโนมัติ 2 ชั้น' 
                : 'Simulate thermal printer status check block over real-time parameters. Test if the drawer triggers correctly.'}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center border-t border-slate-800 pt-6">
            ...
            <div className="md:col-span-5 space-y-4">
              ...
              <div className="space-y-2 text-xs">
                <div className={`flex items-center space-x-2.5 p-3 rounded-xl border ${
                  printerDiagnosticStatus === 'checking' && activeDiagnosticStep === 1 
                    ? 'border-blue-500 bg-blue-500/10' : 'border-slate-800 bg-slate-950/40 text-slate-400'
                }`}>
                  <div className="w-5 h-5 rounded-full bg-slate-800 flex items-center justify-center text-[10px] font-bold">1</div>
                  <span>{isTH ? 'เช็คการจ่ายกระแสไฟเข้า (Volt)' : 'Verify EMF trigger voltage'}</span>
                </div>

                <div className={`flex items-center space-x-2.5 p-3 rounded-xl border ${
                  printerDiagnosticStatus === 'checking' && activeDiagnosticStep === 2 
                    ? 'border-blue-500 bg-blue-500/10' : 'border-slate-800 bg-slate-950/40 text-slate-400'
                }`}>
                  <div className="w-5 h-5 rounded-full bg-slate-800 flex items-center justify-center text-[10px] font-bold">2</div>
                  <span>{isTH ? 'ส่งโค้ดสัญญาณ ESC/POS' : 'Pulse ESC/POS drawer kick code'}</span>
                </div>

                <div className={`flex items-center space-x-2.5 p-3 rounded-xl border ${
                  printerDiagnosticStatus === 'checking' && activeDiagnosticStep === 3 
                    ? 'border-blue-500 bg-blue-500/10' : 'border-slate-800 bg-slate-950/40 text-slate-400'
                }`}>
                  <div className="w-5 h-5 rounded-full bg-slate-800 flex items-center justify-center text-[10px] font-bold">3</div>
                  <span>{isTH ? 'ตรวจเช็คสปริงของลิ้นชักชั้นบน-ล่าง' : 'Sense solenoid trigger feedback'}</span>
                </div>
              </div>
            ...
            {/* Right diagnostic visual feedback logs */}
              ...
              <div className="space-y-1 text-[10px] text-slate-400 overflow-y-auto flex-1">
                ...
                {printerDiagnosticStatus === 'success' && (
                  <>
                    ...
                    <p className="text-green-400 font-bold">&gt; [SUCCESS] Connection established! Dual drawer kicked successfully!</p>
                  </>
                )}
              </div>
            ...
            >
              <div className="p-6 bg-slate-900 text-white flex justify-between items-center">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-sans font-bold text-sm truncate max-w-[280px]">
                    {activeGuide.title}
                  </span>
                </div>
                <button 
